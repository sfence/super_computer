...
super_computer.super_computer_one_slot = appliances.appliance:new(
    {
      node_name_inactive = "super_computer:rack_one_slot",
      node_name_active = "super_computer:rack_one_slot_active",
...
local super_computer_one_slot = super_computer.super_computer_one_slot

super_computer_one_slot:power_data_register(
  {
    ["LV_power"] = {
...
super_computer_one_slot:control_data_register(
  {
    ["punch_control"] = {
      },
  })
...
function super_computer_one_slot:get_formspec(meta, production_percent, consumption_percent)
...
super_computer_one_slot.need_wait = super_computer.super_computer.need_wait

function super_computer_one_slot:cb_allow_metadata_inventory_put(pos, listname, index, stack, player)
  return super_computer.super_computer.allow_metadata_inventory_put(pos, listname, index, stack, player)
end
function super_computer_one_slot:cb_on_metadata_inventory_put(pos, listname, index, stack, player)
  local meta = minetest.get_meta(pos)
  local inv = meta:get_inventory()
  if (inv:get_stack(self.use_stack, 1):get_count()>0)
      and (inv:get_stack(self.use_stack, 2):get_count()>0) then
    local node = minetest.get_node(pos)
    node.name = "super_computer:rack_two_slots"
    minetest.swap_node(pos, node)
  end
end
function super_computer_one_slot:cb_on_metadata_inventory_take(pos, listname, index, stack, player)
  
  local meta = minetest.get_meta(pos)
  self:deactivate(pos, meta)
  local node = minetest.get_node(pos)
  node.name = "super_computer:rack"
  minetest.swap_node(pos, node)
end

function super_computer_one_slot:cb_on_production(timer_step)
  super_computer.super_computer.on_production(self, timer_step)
end

function super_computer_one_slot:cb_waiting(pos, meta)
  super_computer.update_mflops_supply(self, pos, 0)
end

function super_computer_one_slot:cb_deactivate(pos, meta)
  super_computer.update_mflops_supply(self, pos, 0)
end
...
local node_def = {
    ...
    sounds = node_sounds,
    drawtype = "mesh",
    mesh = "super_computer_one_slot.obj",
...
super_computer_one_slot:register_nodes(node_def, node_inactive, node_active)
